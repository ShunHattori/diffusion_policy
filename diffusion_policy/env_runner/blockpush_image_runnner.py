...
class BlockPushModImageRunner(BaseImageRunner):
    def __init__(
        self,
        output_dir,
        n_train=10,
    ...
    def run(self, policy: BaseImagePolicy):
        ...
        for chunk_idx in range(n_chunks):
            ...
            pbar = tqdm.tqdm(
                ...
                desc=f"Eval BlockPushImageRunner {chunk_idx+1}/{n_chunks}",
                leave=False,
                mininterval=self.tqdm_interval_sec,
            )
            done = False
            while not done:
                # TODO ここでobsの処理を実装する(環境を動かして確認する) imageではflattenしてないが・・・？そのまま渡しても良いのか？
                # 基本的にobsの形はconfigに書かれているmeta shapeが参照される．　つまり，このファイルに実装されている処理は汎用的
                # blockpushingのlowdimではmeta shapeはどう定義されている？→無い．obs_dimのみが定義されていた．
                #
