...
class BlockPushEELowdimDataset(BaseLowdimDataset):
    def __init__(
        self,
        zarr_path,
        horizon=1,
    ...
    def get_normalizer(self, mode="limits", **kwargs):
        ...
        if not self.use_manual_normalizer:  # 　だがしかし，configではデフォルトでFalseである
    ...
    def _sample_to_data(self, sample):
        ...
        if not self.obs_eef_target:
            obs[:, 8:10] = 0  # same as the un-modded script
        data = {
            "obs": obs,
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        # samplerはリプレイバッファから取り出すから，EEの情報位限定する必要はないか
