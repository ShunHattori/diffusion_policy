...
from diffusion_policy.env.block_pushing_ee import block_pushing
from diffusion_policy.env.block_pushing_ee.utils import utils_pybullet
from diffusion_policy.env.block_pushing_ee.utils.pose3d import Pose3d
from diffusion_policy.env.block_pushing_ee.utils.utils_pybullet import (
    ObjState,
    XarmState,
)
...
class BlockPushMultimodal(block_pushing.BlockPush):
    ...
    def _reset_block_poses(self, workspace_center_x):
        ...
        def _reset_block_pose(idx, add=0.0, avoid=None):
            def _get_random_translation():
                block_x = workspace_center_x + add
                block_y = -0.2
            ...
            if avoid is None:
                ...
                for _ in range(NUM_RESET_ATTEMPTS):
                    ...
                    if dist > MIN_BLOCK_DIST:
                        break
            block_rotation = transform.Rotation.from_rotvec([0, 0, math.pi])
    ...
    def _reset_target_poses(self, workspace_center_x):
        ...
        def _reset_target_pose(idx, add=0.0, avoid=None):
            def _get_random_translation():
                # Choose x,y randomly.
                target_x = workspace_center_x + add
                target_y = 0.2
